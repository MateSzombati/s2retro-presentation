...
public class ValueRepository : IValueRepository
{
    ...
    public async Task<CategoryValue?> GetByIdAsync(int id) =>
        await _db.Values.FirstOrDefaultAsync(v => v.Id == id);

    public async Task<List<CategoryValue>> GetByCategoryIdAsync(int categoryId) =>
        await _db.Values.Where(v => v.CategoryId == categoryId).ToListAsync();

    public async Task AddAsync(CategoryValue value) => await _db.Values.AddAsync(value);

    public void Update(CategoryValue value) => _db.Values.Update(value);

    public void Delete(CategoryValue value) => _db.Values.Remove(value);
